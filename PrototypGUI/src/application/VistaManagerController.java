 ...
 */
public class VistaManagerController implements VistaContainable{
    ...
    public void init(VistaContainer parent){
        ...
        lvWorkers.setItems(ols);
    }

    @FXML
    private ListView<String> lvWorkers;
    @FXML
    ...
    @FXML
    private ListView<String> lvReports;
    @FXML
        ...
        contestantsPane.expandedProperty().addListener(new InvalidationListener() {
            ...
            public void invalidated(Observable observable) {
                if(contestantsPane.isExpanded()){
                    ObservableList<String> ols = FXCollections.observableArrayList();
                    ols.add("** Dodaj **");
                    ols.add("** Sprawdź godziny **");
                    ols.add("Jakub Pranica");
                    ols.add("Robert Trojan");
                    lvWorkers.setItems(ols);
                }else {
                    lvWorkers.getSelectionModel().clearSelection();
                }
            }
        });

        lvWorkers.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<String>() {
            @Override
            public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
                String s = lvWorkers.getSelectionModel().getSelectedItem();
                if(s==null)
                    return;

                if(s.equals("** Dodaj **")){
                    new VistaRegistrationController(newTab("Zarejestruj nowe konto"));
                    return;
                }else if(s.equals("** Sprawdź godziny **")){
                    new VistaHoursWorkedController(newTab("Sprawdź przepracowane godziny"));
                }

                VistaEntryViewerController entryViewer = new VistaEntryViewerController(newTab("Profil - Jakub Pranica"), "Contestant");
                entryViewer.addEntry("Id", "1" );
                entryViewer.addEntry("Imię", "Jakub" );
                entryViewer.addEntry("Nazwisko", "Pranica" );
                entryViewer.addEntry("Przepracowane godziny", "10" );
                entryViewer.addEntry("Stawka godzinowa", "15zł" );
                entryViewer.addEntry("Informacje kontaktowe", "kubpicapf@gmail.com" );
                entryViewer.addEntry("Informacje dodatkowe", "" );
            }
        });
        ...
        arenasPane.expandedProperty().addListener(new InvalidationListener() {
            ...
            public void invalidated(Observable observable) {
                if(arenasPane.isExpanded()){
                    ObservableList<String> ols = FXCollections.observableArrayList();
                    ols.add("** Dodaj **");
                    lvReports.setItems(ols);
                }else {
                    lvReports.getSelectionModel().clearSelection();
                }
            }
        });

        lvReports.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<String>() {
            @Override
            public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
                String s = lvReports.getSelectionModel().getSelectedItem();
                if(s==null)
                    return;

                if(s.equals("** Dodaj **")){
                    new VistaReportGeneratorController(newTab("Wygeneruj nowy raport"));
                }
            }
        });
